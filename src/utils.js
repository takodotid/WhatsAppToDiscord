...
const updater = {
    ...
    async validateSignature(signature, name) {
        return crypto.verify("RSA-SHA256", fs.readFileSync(name), this.publicKey, signature);
    },
...
const discord = {
    ...
    async findAvailableName(dir, fileName) {
        let absPath;
        const parsedFName = path.parse(fileName);
        let counter = -1;
        do {
...
const whatsapp = {
    ...
    jidToName(jid, pushName) {
        if (this.isMe(state.waClient.user.id, jid)) {
            return "You";
        }
